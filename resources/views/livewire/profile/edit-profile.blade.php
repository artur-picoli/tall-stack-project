    <x-default-card>
        ...
        <div class="flex gap-3 mb-2">
            @if ($photo && !$errors->has('photo'))
                <img wire:loading.attr="hidden" wire:target="photo" class="w-12 h-12 rounded-full" src="{{ $photo->temporaryUrl() }}" alt="Extra large avatar">
            @elseif($userPhoto)
                <img wire:loading.attr="hidden" wire:target="photo" class="w-12 h-12 rounded-full" src="{{ asset($userPhoto) }}" alt="Extra large avatar">
            @else
                <div wire:loading.attr="hidden" wire:target="photo" class="relative w-10 h-10 overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600">
