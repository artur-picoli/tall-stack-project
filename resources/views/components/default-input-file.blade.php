<div>
    <label
        class="block mb-2 text-sm font-medium @error($wireModel ?? $name) text-red-700 dark:text-red-500 @else text-gray-900 dark:text-white @enderror "
        for="{{ $name }}">{{ $label }}
        <svg wire:loading wire:target="{{ $name }}" aria-hidden="true" role="status"
